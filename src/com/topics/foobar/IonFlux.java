...
Output: -1,7,6,3
*/
public class GoogleIonFlux {
    static int count;
    static Map<Integer, Integer> parentMap;

    public static void main(String... args) {
        int[] res = experiment(3, new int[]{1, 4, 7});
        System.out.println(Arrays.toString(res));
    }

    static int[] experiment(int height, int[] nums) {
        count = 0;
        ...
        generatePostOrderTree(height);

        int[] res = new int[nums.length];
        for (int i = 0; i < nums.length; ++i) {
            res[i] = parentMap.get(nums[i]);
        }

        return res;
    }

    static int generatePostOrderTree(int height) {
        if (height == 0) {
            return 0;
        }

        int left = generatePostOrderTree(height-1);
        int right = generatePostOrderTree(height-1);
        int parent = ++count;
        if (parentMap.containsKey(left)) {
            parentMap.put(left, parent);
        }
        if (parentMap.containsKey(right)) {
            parentMap.put(right, parent);
        }
